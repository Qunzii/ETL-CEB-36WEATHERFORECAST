...
def parseAPI(connObj):
    ...
    for data_seq in xml_list:
        ...
        delVariable = delVariable.replace('\'', '')
        delData = '''delete from cwb.temp_opendata_cwb_36weatherforecast
        ...
        insVariable = insVariable.replace('\'', '')
        insCollection = insVariable * int(len(result_collection) / 11)
        insCollection = insCollection.replace(')(', '), (')
        insData = '''INSERT INTO cwb.temp_opendata_cwb_36weatherforecast
                        (opendata_id, issued_time, city, start_time, end_time, wx, maxt, mint, ci, pop, lastupddate)
        ...
        main_delData = '''delete from cwb.main_opendata_cwb_36weatherforecast
                            where opendata_id = %s;'''
        ...
        main_insData = '''insert into cwb.main_opendata_cwb_36weatherforecast
                            select * from cwb.temp_opendata_cwb_36weatherforecast where opendata_id = %s;'''
